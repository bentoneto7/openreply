...
import { AlertCircle, ArrowUpRight, Clock3, Flame, Handshake, MessageCircle, RefreshCw, Search, Send, Users } from "lucide-react";

type Period = "24h" | "7d" | "30d" | "90d";
interface QueueItem { key: string; instagramUsername: string; commenterId: string; commenterName: string | null; latestComment: string; latestKeyword: string | null; automationName: string; lastSeenAt: string; signalCount: number; sentCount: number; score: number; temperature: LeadTemperature; leadStatus: LeadStatusValue; reasons: string[] }
interface HeatmapData { accounts: AccountOption[]; queue: QueueItem[]; truncated: boolean; temperatureCounts: Record<LeadTemperature, number>; metrics: { triggeredComments: number; uniquePeople: number; awaitingReview: number; openOpportunities: number; automaticDmsSent: number } }
...
export default function HeatmapPage() {
  ...
  return <div className="space-y-7">
    ...
    {loading && <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5" aria-label="Carregando mapa de calor">{Array.from({ length: 5 }, (_, index) => <div key={index} className="panel h-36 animate-pulse rounded p-5" />)}</div>}
    ...
    {!loading && data && <>
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5" aria-label="Indicadores do período">
        ...
        <Metric icon={Users} label="Pessoas únicas" value={data.metrics.uniquePeople} helper="Quem comentou ou mandou mensagem, contado por conta do Instagram." />
        <Metric icon={Search} label="Aguardando abordagem" value={data.metrics.awaitingReview} helper="Pessoas com sinal recente que ninguém marcou como abordada." />
        <Metric icon={Handshake} label="Em andamento" value={data.metrics.openOpportunities} helper="Leads que alguém já moveu no CRM e ainda estão abertos." />

...
"GUIA","Aqui está: {link}","Enviei por DM!","https://exemplo.com.br/guia","Olá! Toque abaixo para receber o material","Enviar link"
"LINK,LOJA","Acesse aqui: {link}","Confira sua DM",,,`;
...
export default function ImportCampaignsPage() {
  ...
  function startImport() {
    ...
    if (parsed.length === 0) {
      setError("Cole um CSV com cabeçalho e pelo menos uma automação.");
      return;
    }
    ...
    for (let i = 0; i < parsed.length; i++) {
      ...
      if (keywords.length === 0 || !dmMessage) {
        setError(`A linha ${i + 1} não possui palavras-chave ou mensagem.`);
        return;
      }
      rows.push({
        name: (r.name ?? "").trim(),
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-muted mt-1">
          Cole um CSV com uma linha por automação. Cada linha será aberta no
          editor já preenchida para você revisar e escolher o reel antes de
          ...
          <code className="text-accent">opening_dm_button</code>. Coloque as
          palavras-chave na mesma célula, separadas por vírgulas. Use{" "}

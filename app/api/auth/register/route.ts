...
export async function POST(request: Request) {
  ...
  if (!parsed.success) return NextResponse.json({ success: false, error: "Confira nome, e-mail e senha. Use pelo menos 8 caracteres, uma letra e um número." }, { status: 400 });
  ...
  try {
    await prisma.$transaction(async (tx) => {
      const user = await tx.user.create({ data: { name: parsed.data.name, email: parsed.data.email, emailVerified: new Date(), passwordHash } });
